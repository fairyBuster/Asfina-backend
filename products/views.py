...
class ProductViewSet(viewsets.ModelViewSet):
    ...
    @extend_schema(
        tags=[USER_TAG],
        parameters=[
            OpenApiParameter(name='min_price', type=float, description='Filter by minimum price'),
            OpenApiParameter(name='max_price', type=float, description='Filter by maximum price'),
        ],
        responses={
            200: OpenApiResponse(
                response=ProductListSerializer(many=True),
                description='List of active products with basic information'
            )
        },
        description="""
        ...
        - ID and name
        - Price and investment range
        - Profit rate and duration
        - Available stock
        
        Use filters to narrow down results by price range.
        """
    )
