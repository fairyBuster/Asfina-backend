...
class RankLevelAdmin(admin.ModelAdmin):
    list_display = ('rank', 'title', 'missions_required_total', 'created_at', 'updated_at')
    list_editable = ('title', 'missions_required_total')
    ordering = ('rank',)
    search_fields = ('title',)
    readonly_fields = ('created_at', 'updated_at')
    fieldsets = (
        ('Konfigurasi Rank', {
            'fields': ('rank', 'title', 'missions_required_total'),
            'description': (
                'Ambang (missions_required_total) akan dihitung sesuai kebijakan di General Settings -> Kebijakan Rank. '
                'Pilih basis: misi selesai, jumlah downline, atau downline aktif; dan tentukan level downline yang dihitung.'
            )
        }),
        ('Waktu', {
            'fields': ('created_at', 'updated_at'),
